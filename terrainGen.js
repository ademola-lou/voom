 ...
 */
THREEx.Terrain.simplexHeightMap	= function(heightMap, image){
  ...
  image.height = depth;
   ...
   var WIDTH = image.width;
	...
	for(var x = 0; x < width; x++){
      ...
      var data = context.getImageData(x, z, 1, 1).data;
      var rgbToLuminance = ((0.2989*data[0])		+		(0.5870*data[1]) +	(0.1140*data[2]));
			...
			heightMap[x][z]	= rgbToLuminance/3000;
		}
	}
}
...
if( !(x >= -4 && x < width) ){
 console.log("out of boun") 
  
}
  if( (z >= -4 && z < width) && ((x >= -4 && x < width))){
...
THREEx.Terrain.heightToColor	= (function(){
	...
	return function(height){
		...
		if( height > 0.08 ){
			height		= (height*2)*0.0005 + 0.2
			color.setRGB(.5,height*5,.5)
		}else if( height < 0.9 ){
			height		= (height-0.5)/0.2
			height		= height*0.5 + 0.2
			color.setRGB(.5,.5, height)
		}else{
			height		= (height-0.7)/0.3
			height		= height*0.5 + 0.5
			color.setRGB(.5,height, .5)
		}
		// color.setRGB(1,1,1)
		return color;		
	}
